...
Recently \cite{Mikolov:13} found that continuous space word representations capture syntactic and semantic regularities. For example they find that $\textit{queen} - \textit{king} \approx \textit{woman}  -\textit{man}$. The authors use these linguistics regularities to answer a set of analogy questions in the form of `$a$ is to $b$ as $c$ is to \dots'. In this framework, every word is represented by exactly one continuous space word representation. One potential problem when answering analogy questions is that words can be ambiguous. When the question is `\textit{seed} is to \textit{apple} as \textit{window} is to \textit{house}' then the word apple refers to a fruit. Another question could be `\textit{apple} is to \textit{computer} as \textit{porsche} is to \textit{cars}' where the company apple is implied. Clearly, these are two very distinct entities, but the approach that Mikolov et al. presented does not differentiate between the two different meanings of apple. We propose to disambiguate between the various senses of the word apple, in order to obtain multiple continuous space words representations for one word. 
...
Let $C$ be the set of words that co-occur with $A$, COCONUT first converts the global co-occurrence vectors of the words in $C$ to relatedness vectors. It will then cluster these relatedness vectors in order to determine the two possibly different meanings of $A$. 
...
\section{Evaluation}
We have evaluated the performance of COCONUT on a dataset containing 60237 unique words, and has size X. Initially, we decided not to disambiguated the top 75 most frequent words, after extracting the two senses of the words and their distance, we discarded half of the disambiguated words, leaving us with X words that were disambiguated. 
...
On dataset X we find for the same word two different clusters: 
